#Data pre-processing
  ...
  #For those who want to understand the algorithmic process, a small dataset is provided in this paper demo
  ...
  # disease_all<-cbind(disease1,disease2[,-1])
  ###############################################
  #                                             #
  # Randomly divide training set and testing set#
  #                                             #
  ###############################################
  # #2:1 Randomly divide the training and test sets
  # set.seed(1230)
  # train_num1<-sample(c(2:length(disease1)),size =(2/3*(length(disease1)-1) ),replace = F)#Take 2/3 as the training set
  # train_num2<-sample(c(1:(length(disease2)-1)+length(disease1)),size =(2/3*(length(disease2)-1) ),replace = F)#Take 2/3 as the training set
  # disease<-disease_all[,c(1,train_num1,train_num2)]
  # testdisease<-disease_all[,-c(train_num1,train_num2)]
  ...
  # cancer_type<-c(rep(1,length(train_num1])),rep(2,length(train_num2)))#train cancer type
  # test_cancer_type<-c(rep(1,1/3*(length(disease1)-1)),rep(2,1/3*(length(disease2)-1)))#train cancer type
...
#The process of building a single gene network(The Methods starts here,there is a little demo data)
#Get current path
#getwd()
#Set the working path to the path of HSSG_code.
#work_path<-"xxx/HSSG-main/HSSG_code/"
#setwd(work_path)

######Create two folders file that need to be used.(/file/ and /similar/)
#####When downloading HSSG, the author has already established two folders, file and similar, in the current path. 
#####If you run this code separately, create it separately.
#dir.create(file.path(getwd(),"file"))
#dir.create(file.path(getwd(),"similar"))
...
###########################################################################################
#                                                                                         #
#Step1£ºConstruction of sample-similarity network based on a single gene                  #
#                                                                                         #
###########################################################################################
...
###########################################################################################
#                                                                                         #
#Step2£ºCalculating heterogeneity score of single gene based on Pseudo-F Statistics       #
#                                                                                         #
###########################################################################################
...
write.csv(data, file = paste("./Fstat(order).csv"))#The calculated contribution of each gene
...
while(i<=length(fD[[1]]))
{
    ...
    acc<-(a[1,1]+a[2,2])/(length(disease)-1)#Methods divided into two categories
    if(acc<0.5) acc<-1-acc
    #p1<-a[1,1]+max((a[2,2]+a[3,3]),(a[2,3]+a[3,2]))#Methods divided into three categories
    #p2<-a[1,2]+max((a[2,1]+a[3,3]),(a[2,3]+a[3,1]))
    #p3<-a[1,3]+max((a[2,1]+a[3,2]),(a[2,2]+a[3,1]))
    #p<-max(p1,p2,p3)
    #acc<-max(p)/(length(disease)-1)
...
###########################################################################################
#                                                                                         #
#Step3£ºConstruction of gene-gene network based on single gene sample-similarity network  #
#                                                                                         #
###########################################################################################
...
###########################################################################################
#                                                                                         #
#Step4£ºNetwork analysis and module mining                                                #
#                                                                                         #
###########################################################################################
...
NGennet<-read_csv("./NGennet.csv")
NGennet<-NGennet[1:Number_selected_build_gene_network,1:Number_selected_build_gene_network]
...
###########################################################################################
#                                                                                         #
#Selecting random gene clustering results                                                 #
#                                                                                         #
###########################################################################################
acc_list<-c()
all_acc<-0
Random_selection_number<-30#The number of randomly selected genes can be adjusted according to the needs.
for(i in 1:50)
{
  set.seed(i+100)
  o<-sample(c(1:length(disease[,1])),size = Random_selection_number,replace = F)
  disease_r<-disease[o,]
  data<-t(as.matrix(disease_r[,-1]))
  Hdata<-data.frame(data)
  names(Hdata)<-disease[o,1]
  Hdata$Species<-cancer_type
  Hdata.kmeans<-kmeans(Hdata[,1:Random_selection_number],2)
  a<-table(Hdata$Species,Hdata.kmeans$cluster)
  acc<-(a[1,1]+a[2,2])/(length(disease))#Methods divided into two categories
  if(acc<0.5) acc<-1-acc 
  all_acc<-(all_acc+acc)
